...
function renderOperationsDashboard(PDO $database, string $notice = '', string $error = '', bool $emailOnly = false): never
{
    ...
    if ($emailOnly) {
        $content = '<header class="dashboard-header"><div><p class="eyebrow">24Seven.FM Player · Closed alpha</p><h1>Coordinator email</h1><p class="muted">Compose individual tester-program email, review the resolved draft, and inspect protected handoff records.</p><p><a class="link-button" href="/private-tester-queue.php">Operations</a><a class="link-button" href="/private-tester-queue.php?live_chat=1">Live Chat</a><a class="link-button" href="/private-tester-queue.php?mail_archive=1">Sent archive (' . $archiveCount . ')</a></p></div><form method="post"><input type="hidden" name="action" value="logout"><input type="hidden" name="csrf" value="' . e(csrf()) . '"><button class="secondary" type="submit">Sign out</button></form></header>' . $message
            ...
            . $composeHint
            . '<script src="/assets/private-tester-queue.js?v=onboarding-3" defer></script>';
    ...
    $content = '<header class="dashboard-header"><div><p class="eyebrow">24Seven.FM Player · Closed alpha</p><h1>Tester operations</h1><p class="muted">A private workspace for moving a new application through review to a focused, safe testing assignment.</p><p><a class="link-button" href="/private-tester-queue.php?live_chat=1">Live Chat</a><a class="link-button" href="/private-tester-queue.php?email=1">Email (' . $archiveCount . ')</a><a class="link-button" href="/private-tester-queue.php?mail_archive=1">Sent archive</a></p></div><form method="post"><input type="hidden" name="action" value="logout"><input type="hidden" name="csrf" value="' . e(csrf()) . '"><button class="secondary" type="submit">Sign out</button></form></header>' . $message
        ...
        . '<div class="operations-panel-toolbar" role="group" aria-label="Operations panel layout"><span>Drag a panel by its title. Drag its lower-right corner to resize.</span><button type="button" data-operations-reset>Reset panel layout</button></div><div class="operations-panel-row" data-operations-panels><section class="applications-card" data-operations-panel="applications"><div class="operations-panel-title" data-panel-drag-handle role="button" tabindex="0" aria-label="Move Applications awaiting review panel"><span class="panel-grip" aria-hidden="true">⋮⋮</span><h2>Applications awaiting review (' . count($pendingApplications) . ')</h2></div>' . ($applicationRows === '' ? '<p class="muted">No new applications right now.</p>' : '<p class="muted">Newly submitted or imported applications stay here, separate from the accepted roster, until a coordinator accepts, requests details, or rejects them.</p><div class="application-rows">' . $applicationRows . '</div>') . '<div class="operations-corner-resize" data-panel-resize-handle role="separator" aria-label="Resize Applications awaiting review panel" aria-orientation="both" tabindex="0"></div></section>'
        . '<section class="roster-card" data-operations-panel="roster"><div class="operations-panel-title" data-panel-drag-handle role="button" tabindex="0" aria-label="Move Tester roster panel"><span class="panel-grip" aria-hidden="true">⋮⋮</span><h2>Tester roster</h2></div><p class="muted">Open one workspace to review coverage, record onboarding progress, send an individual orientation email, and manage focused tasks.</p><div class="roster-list">' . ($rows === '' ? '<p class="muted">No accepted testers yet.</p>' : $rows) . '</div><div class="operations-corner-resize" data-panel-resize-handle role="separator" aria-label="Resize Tester roster panel" aria-orientation="both" tabindex="0"></div></section></div>'
        . '</div>'
        . '<script src="/assets/private-tester-queue.js?v=onboarding-3" defer></script>';

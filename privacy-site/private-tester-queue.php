...
/**
 * Keeps the activity evidence readable when a browser has a stale shared CSS
 * asset. This is intentionally limited to the activity component's structure.
 */
...
/**
 * Renders an evidence-only timeline for the tester and their coordinator.
 * Every entry comes from an existing protected record. It deliberately does
 * not infer profile completion, installation, inbox delivery, or a decision
...
function testerActivityTimeline(PDO $database, array $tester, array $assignments, array $tasks, string $viewer): string
{
    ...
    $feedback = $database->prepare('SELECT feedback.pt_case, feedback.outcome, feedback.category, feedback.created_at, assignments.task_id FROM tester_feedback AS feedback LEFT JOIN tester_task_assignments AS assignments ON assignments.id = feedback.assignment_id WHERE feedback.tester_id = ? ORDER BY feedback.created_at DESC, feedback.id DESC');
    $feedback->execute([(int) $tester['id']]);
    foreach ($feedback->fetchAll() as $report) {
    ...
    if ($events === []) {
        return '<section class="activity-timeline"><p class="eyebrow">Activity</p><h2>Timeline</h2><p class="muted">No protected onboarding or task activity has been recorded yet.</p></section>';
    }
    $items = '';
...
function renderOperationsDashboard(PDO $database, string $notice = '', string $error = '', bool $emailOnly = false): never
{
    ...
    $privateEmailContext = '<script id="private-tester-email-context" type="application/json">' . json_encode(['mobileOptInUrl' => 'https://play.google.com/apps/testing/com.codeframe78.twentyfourseven.player'], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES) . '</script>';

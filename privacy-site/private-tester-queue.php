...
function renderOperationsDashboard(PDO $database, string $notice = '', string $error = '', bool $emailOnly = false): never
{
    ...
    $content = '<header class="dashboard-header"><div><p class="eyebrow">24Seven.FM Player · Closed alpha</p><h1>Tester operations</h1><p class="muted">A private workspace for moving a new application through review to a focused, safe testing assignment.</p><p><a class="link-button" href="/private-tester-queue.php?live_chat=1">Live Chat</a><a class="link-button" href="/private-tester-queue.php?email=1">Email (' . $archiveCount . ')</a><a class="link-button" href="/private-tester-queue.php?mail_archive=1">Sent archive</a></p></div><form method="post"><input type="hidden" name="action" value="logout"><input type="hidden" name="csrf" value="' . e(csrf()) . '"><button class="secondary" type="submit">Sign out</button></form></header>' . $message
        ...
        . '<div class="operations-panel-toolbar" role="group" aria-label="Arrange operations panels"><span>Arrange panels</span><button type="button" data-operations-swap>Swap sides</button><button type="button" data-operations-wide="applications">Widen applications</button><button type="button" data-operations-wide="roster">Widen roster</button><button type="button" data-operations-reset>Reset</button></div><div class="operations-panel-row" data-operations-panels data-order="applications-first"><section class="applications-card"><h2>Applications awaiting review (' . count($pendingApplications) . ')</h2>' . ($applicationRows === '' ? '<p class="muted">No new applications right now.</p>' : '<p class="muted">Newly submitted or imported applications stay here, separate from the accepted roster, until a coordinator accepts, requests details, or rejects them.</p><div class="application-rows">' . $applicationRows . '</div>') . '</section><div class="operations-resize-handle" data-operations-resize role="separator" aria-label="Resize applications and tester roster panels" aria-orientation="vertical" aria-valuemin="35" aria-valuemax="70" tabindex="0"></div>'
        . '<section class="roster-card"><h2>Tester roster</h2><p class="muted">Open one workspace to review coverage, record onboarding progress, send an individual orientation email, and manage focused tasks.</p><div class="roster-list">' . ($rows === '' ? '<p class="muted">No accepted testers yet.</p>' : $rows) . '</div></section></div>'
        . '</div>'
        . '<script src="/assets/private-tester-queue.js?v=onboarding-2" defer></script>';

...
function renderDashboard(PDO $database, string $notice = '', string $error = ''): never
{
    ...
    $content = '<div class="actions"><div><h1>Private tester queue</h1><p class="muted">' . count($testers) . ' active tester' . (count($testers) === 1 ? '' : 's') . '. Each send is delivered individually; recipients are never exposed to one another.</p></div><form method="post"><input type="hidden" name="action" value="logout"><input type="hidden" name="csrf" value="' . e(csrf()) . '"><button class="secondary" type="submit">Sign out</button></form></div>' . $message
        . '<form method="post" id="email-form"><input type="hidden" name="action" value="prepare"><input type="hidden" name="csrf" value="' . e(csrf()) . '"><section><h2>Active testers</h2><p><label><input type="checkbox" id="select-all"> Select all active testers</label></p><table><thead><tr><th scope="col">Select</th><th scope="col">Tester</th><th scope="col">Device</th><th class="optional" scope="col">Coverage</th><th class="optional" scope="col">Experience</th></tr></thead><tbody>' . $rows . '</tbody></table></section><section><h2>Prepare HTML email</h2><p class="muted">Use the formatting bar, then review the selected recipients and rendered message before any delivery is attempted. A plain-text alternative is included for mail clients that cannot show HTML.</p><label for="subject">Subject</label><input id="subject" name="subject" maxlength="' . MAX_SUBJECT_LENGTH . '" required>' . $editor . '<button type="submit">Review selected recipients</button></section></form><script>(function(){var all=document.getElementById("select-all"),editor=document.getElementById("body-editor"),hidden=document.getElementById("body-html"),form=document.getElementById("email-form"),savedRange=null;function saveRange(){var selection=window.getSelection();if(!selection||!selection.rangeCount||!editor.contains(selection.anchorNode)){return;}savedRange=selection.getRangeAt(0).cloneRange();}function restoreRange(){if(!savedRange){editor.focus();return;}var selection=window.getSelection();selection.removeAllRanges();selection.addRange(savedRange);editor.focus();}all.addEventListener("change",function(){document.querySelectorAll("input[name=\"tester_ids[]\"]").forEach(function(box){box.checked=all.checked;});});editor.addEventListener("keyup",saveRange);editor.addEventListener("mouseup",saveRange);editor.addEventListener("input",saveRange);document.addEventListener("selectionchange",saveRange);document.querySelectorAll("[data-format]").forEach(function(button){button.addEventListener("mousedown",function(event){event.preventDefault();});button.addEventListener("click",function(){var command=button.dataset.format;if(command==="createLink"){var url=window.prompt("Link URL (http, https, or mailto):");if(url){restoreRange();document.execCommand(command,false,url);saveRange();}}else{restoreRange();document.execCommand(command,false,null);saveRange();}});});form.addEventListener("submit",function(event){hidden.value=editor.innerHTML;if(!editor.textContent.trim()){event.preventDefault();editor.focus();alert("Write an email message before reviewing recipients.");}});}());</script>';

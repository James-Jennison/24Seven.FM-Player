...
const PORTAL_REQUEST_COOLDOWN_SECONDS = 900;
...
function portalSaveProfile(PDO $database, array $tester): void
{
    ...
    $update = $database->prepare('UPDATE testers SET device = ?, android_version = ?, primary_station = ?, station_accounts_json = ?, device_form_factor = ?, other_devices = ?, network_capabilities_json = ?, audio_capabilities_json = ?, accessibility_capabilities_json = ?, testing_comfort = ?, controlled_actions_json = ?, testing_availability = ? WHERE id = ?');
    $update->execute([portalText('device', 160, true), portalText('android_version', 48, true), $primary, json_encode($accounts, JSON_THROW_ON_ERROR), $deviceType, portalText('other_devices', 500), json_encode($network, JSON_THROW_ON_ERROR), json_encode($audio, JSON_THROW_ON_ERROR), json_encode($accessibility, JSON_THROW_ON_ERROR), $comfort, json_encode($controlled, JSON_THROW_ON_ERROR), $availability, (int) $tester['id']]);
    portalRedirect('?notice=' . rawurlencode('Your profile and device details are saved.'));
}

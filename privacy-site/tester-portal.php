...
function portalWorkspaceNavigation(string $activeView, ?int $previewTesterId = null): string
{
    ...
    foreach ($items as $view => $label) {
        ...
        $links .= '<a class="tester-workspace-link' . $active . '" href="' . e(portalWorkspaceUrl($view, $previewTesterId)) . '"' . ($view === $activeView ? ' aria-current="page"' : '') . '>' . e($label) . '</a>';
    }
    return '<nav class="tester-workspace-nav" aria-label="Tester workspace">' . $links . '</nav>';
}
...
function portalPage(string $title, string $content): never
{
    ...
    $page = str_replace('</style></head><body><main class="shell">', '</style><link rel="stylesheet" href="/assets/onboarding-portal.css?v=' . e($portalStyleVersion) . '"><link rel="stylesheet" href="/assets/onboarding-wizard.css?v=' . e($wizardStyleVersion) . '"></head><body><a class="skip-link" href="#tester-portal">Skip to tester portal</a><main id="tester-portal" class="portal-preview-shell">', $page);
    ...
    $page = str_replace('</main></body></html>', '<script src="/assets/onboarding-wizard.js?v=' . e($wizardVersion) . '" defer></script></main></body></html>', $page);
    echo $page;
    exit;
}
...
function portalRenderDashboard(PDO $database, array $tester, bool $adminPreview = false, string $coordinatorName = '24Seven.FM Coordinator'): never
{
    ...
    $dashboardPanel = '<section class="card hero"><p class="eyebrow">Tester dashboard</p><h1>Welcome, ' . e($tester['display_name']) . '</h1><p class="muted">Your dashboard is a short status overview. Use the workspace navigation for onboarding, coverage, focused tasks, structured reports, and private support. Guest testing does not require a 24Seven.FM station account.</p><a class="button" href="' . e(portalWorkspaceUrl($nextView, $previewTesterId)) . '">' . e($nextLabel) . '</a></section>' . $progressChecklist
        . '<section class="card"><p class="eyebrow">Workspace</p><h2>Go directly to the page you need</h2><div class="workspace-action-grid"><a href="' . e(portalWorkspaceUrl('profile', $previewTesterId)) . '"><strong>Profile &amp; Device</strong><span>Update your coverage and registered setup.</span></a><a href="' . e(portalWorkspaceUrl('tasks', $previewTesterId)) . '"><strong>My Tasks</strong><span>Read your assigned instructions and safety boundary.</span></a><a href="' . e(portalWorkspaceUrl('reports', $previewTesterId)) . '"><strong>Report results</strong><span>Record one outcome for each assigned PT case.</span></a><a href="' . e(portalWorkspaceUrl('support', $previewTesterId)) . '"><strong>Support</strong><span>Open your private Coordinator conversation.</span></a></div></section>';

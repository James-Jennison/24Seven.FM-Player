...
- Latest local M23 refinement: trusted same-station chat emoticons render inline with text fallbacks, submitted phpBB emoticon codes are compared against their visible server form, and More prioritizes the selected account with compact disclosures for lower-frequency controls. The catalog now exposes only the mobile-relevant Contact and Membership browser links. Debug compilation, all 111 unit tests, lint, and Android-test compilation pass; refreshed Chat and More screenshots are included.

...
Updated August 11, 2026. The five account systems are independent even where their legacy pages and field names are similar. Registration/recovery/management paths below are protocol inventory only and are not linked by the M31 Play artifact. No credential, active cookie, anti-spam answer, private identifier, or authentication header is recorded here.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| StreamingSoundtracks.com (`sst`) | HTTPS account form supplies a same-origin text anti-spam prompt and answer field | `/modules.php?name=Your_Account&op=new_user` | Public account flow; exact recovery route not yet separately verified | `/modules.php?name=Your_Account` and `op=edituser` | `op=logout` | `username`, transient password, server-provided answer field and hidden fields; station cookies; same-origin redirects; encrypted restored session | Text anti-spam migration implemented; needs user-entered production acceptance |
| 1980s.FM (`1980s`) | Independent HTTPS account form supplies the current text anti-spam check | Same station-relative registration module | Exact recovery route not yet separately verified | Same station-relative account/edit module | Same station-relative logout verified | Separate station ID, expected host, cookie manager entry, encrypted storage key, and auth state; server-provided form fields are submitted unchanged | Text anti-spam migration implemented; needs user-entered production acceptance |
| Adagio.FM (`adagio`) | Independent HTTPS account form supplies the current text anti-spam check | Same station-relative registration module | Exact recovery route not yet separately verified | Same station-relative account/edit module | Same station-relative logout verified | Separate station ID, expected host, cookie manager entry, encrypted storage key, and auth state; server-provided form fields are submitted unchanged | Text anti-spam migration implemented; needs user-entered production acceptance |
| Death.FM (`death`) | Independent HTTPS account form supplies the current text anti-spam check | Same station-relative registration module | Exact recovery route not yet separately verified | Same station-relative account/edit module | Same station-relative logout verified | Separate station ID, expected host, cookie manager entry, encrypted storage key, and auth state; server-provided form fields are submitted unchanged | Text anti-spam migration implemented; needs user-entered production acceptance |
| Entranced.FM (`entranced`) | Independent HTTPS account form supplies the current text anti-spam check | Same station-relative registration module | Exact recovery route not yet separately verified | Same station-relative account/edit module | Same station-relative logout verified | Separate station ID, expected host, cookie manager entry, encrypted storage key, and auth state; server-provided form fields are submitted unchanged | Text anti-spam migration implemented; needs user-entered production acceptance |
...
- Passwords and anti-spam answers are transient method parameters and are not written to Room, DataStore, preferences, logs, resources, build files, or documentation.

...
- The Ubuntu protected-signing helper consumes that same authenticated recovery format without changing the upload identity, rejects the wrong passphrase/hash/certificate and repository-local packages, keeps its mode-`600` keystore only on memory-backed `/dev/shm`, uses a non-persistent Gradle process, and verifies the resulting AAB signer. Five synthetic tests pass; the production package and owner-held passphrase have not yet been used on Linux.

...
| Native login | Yes | Yes | Yes | Yes | Yes | Yes | N/A | No | Legacy form/text anti-spam check | No | Text anti-spam migration implemented; user-entered production acceptance pending | Sessions isolated by station ID and host |
...
| System audio-output selection | Existing verified stream | Same | Same | Same | Same | Current route state only | No for listening | No | Android system Output Switcher | No | Dedicated Android output path implemented in M25 | Device/Bluetooth/wired/USB and supported system routes use the existing Media3 session; no scan, relay, proxy, or new endpoint. Google Cast is deferred optional M55 scope |
...
| Community notifications | Local Chat mention detection; optional foreground monitor supports one selected signed-in station | Same | Same | Same | Same | Yes for local station opt-in | Yes for signed-in identity | No | Existing ClearChat source; future event source or privacy-compatible relay remains M36–M38 | One Chat fetch about every 60 seconds only while the visible monitor is enabled | Foreground-monitor implementation pending device acceptance; M36–M38 still pending | Exact-name matching, baseline, blocked-author filtering, bounded memory-only dedupe, private no-message-text alert, gated Chat navigation, persistent Stop control, and no session forwarding |

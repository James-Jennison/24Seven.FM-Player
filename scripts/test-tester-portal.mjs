...
assert(queue.includes('data-activity-kind') && queue.includes('data-activity-filter="communication"') && queue.includes('generic messages were recorded') && portal.includes('/assets/activity-timeline.js?v='), "Activity timelines must offer read-only category filters, group generic mail handoffs, and load the local-time enhancement.");
...
assert(portal.includes('tester_retest_handoffs AS handoffs') && portal.includes('assignmentRequiredPtCases($database, $assignmentId, $task)') && portal.includes('This is a separate re-test for'), "A separate re-test must show its exact PT case and Tester-visible instruction without reopening the original assignment.");
...
assert(queue.includes('function assignmentLifecycleMarkup') && portal.includes("assignmentLifecycleMarkup($assignment, $progress, 'tester', $latestReview)") && queue.includes("assignmentLifecycleMarkup($assignment, $progress, 'coordinator', $latestReview)"), "Tester and Coordinator assignment records must share the same evidence-backed lifecycle presentation.");
assert(queue.includes('function renderWorkReviewInbox') && queue.includes('?work_review=1') && queue.includes("isset($_GET['work_review'])") && queue.includes('Submitted Tester work') && queue.includes('Return for clarification'), "Coordinator operations must expose a dedicated submitted-work review inbox with explicit decisions.");
assert(queue.includes("action\" value=\"review_assignment") && queue.includes('tester_task_review_events') && queue.includes('tester_task_review_responses'), "Coordinator decisions and returned-work clarification responses must retain dedicated auditable evidence.");
assert(portal.includes('function portalSubmitReviewClarification') && portal.includes('submit_review_clarification') && portal.includes('Coordinator clarification requested') && portal.includes('Submit clarification for review'), "A returned assignment must give the Tester an explicit, auditable clarification path rather than reopening duplicate PT-case reports.");

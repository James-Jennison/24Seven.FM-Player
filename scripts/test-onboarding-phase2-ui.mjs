...
assert(queue.includes("hash_file('sha256', $portalStyleAsset)") && queue.includes('<b aria-hidden="true">▦</b><span>Operations</span>') && queue.includes('<b aria-hidden="true">◌</b><span>Live Chat</span>'), "Coordinator workspace navigation must use the same cache-versioned icon-and-label rail markup as the Tester portal.");
...
assert(queue.includes('workspace-page-header') && queue.includes('workspace-attention-grid') && queue.includes('workspace-queue-grid'), "Coordinator Operations must present the shared application page anatomy, attention queue, and record queues.");
...
assert(portal.includes('workspace-page-header') && portal.includes('workspace-primary-card') && portal.includes('workspace-attention-grid'), "Tester routes must present the shared application page anatomy and a next-action workbench.");
assert(portal.includes('work-record-progress') && portal.includes('Report a PT case') && portal.includes('Evidence queue'), "Tester tasks and reports must expose a focused work record, per-case progress, and explicit reporting path.");
assert(queue.includes('coordinator-record-header') && queue.includes('Coordinator record') && queue.includes('record-workbench-section'), "Coordinator tester records must use the same focused record-page anatomy as the Tester workspace.");
...
assert(workspaceStyles.includes('flex:0 0 12.25rem') && workspaceStyles.includes('grid-template-columns:1.35rem minmax(0,1fr)') && workspaceStyles.includes('.rail-button span{font-size:.78rem'), "Desktop workspace navigation must keep route labels readable beside their icons.");
...
assert(workspaceStyles.includes('.workspace-page-header') && workspaceStyles.includes('.workspace-primary-card') && workspaceStyles.includes('.workspace-queue-grid'), "The shared style system must support consistent page anatomy, attention cards, and Coordinator record queues.");
assert(workspaceStyles.includes('.work-record-progress') && workspaceStyles.includes('.record-workbench-section'), "The shared style system must support task evidence progress and Coordinator record pages.");

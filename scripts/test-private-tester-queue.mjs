...
assert(endpoint.includes('function coordinatorCoverageCatalog(PDO $database): array') && endpoint.includes('function renderCoverageCatalog(PDO $database): never'), "Coordinator coverage must be derived by a dedicated read-only catalog contract.");
assert(endpoint.includes('?coverage=1') && endpoint.includes('Coverage catalog') && endpoint.includes('Available capacity'), "Coordinator navigation must expose a separate Coverage Catalog with assignment capacity.");
assert(endpoint.includes('the catalog does not create or update a Tester record.') && endpoint.includes('It is advisory only: it neither confirms app use nor sends mail, assigns work, or changes profile data.'), "Coverage Catalog privacy and non-mutation boundaries must be explicit.");
assert(endpoint.includes("const ISSUE_TRIAGE_STATES = ['needs_reproduction', 'known_issue', 'ready_for_retest', 'closed']"), "Issue triage must use only the bounded Coordinator states.");
...
assert(endpoint.includes('function retestNotificationEligible(PDO $database') && endpoint.includes('function sendRetestNotification(PDO $database'), "Only a separate re-test with no prior notification attempt may use the explicit notification action.");
assert(endpoint.includes('name="action" value="send_retest_notification"') && endpoint.includes('name="confirm_retest_notification" value="yes"'), "A re-test notification must require a deliberate Coordinator confirmation from its dedicated review screen.");
assert(endpoint.includes("'retest_notification' => 'Re-test notification'") && endpoint.includes("'retest_notification'"), "A re-test notification must preserve a distinct protected mail-archive type.");
assert(endpoint.includes('This does not prove inbox delivery or reading') && endpoint.includes('No automatic retry will occur.'), "The Coordinator must be told that mail transport acceptance is not delivery proof and no automatic retry occurs.");
assert(endpoint.includes('Complete only the exact PT case listed above') && endpoint.includes('tester-portal.php?view=tasks'), "The notification preview must limit the Tester to the exact re-test case and protected portal destination.");
assert(endpoint.includes('CREATE TABLE IF NOT EXISTS tester_retest_closeouts') && endpoint.includes('function recordRetestCloseout(PDO $database'), "A completed re-test must retain one linked, immutable closeout record.");
assert(endpoint.includes('function renderRetestCloseoutReview(PDO $database') && endpoint.includes('record_retest_closeout') && endpoint.includes('confirm_retest_closeout'), "Re-test closeout must use a dedicated Coordinator review page and confirmation.");
assert(endpoint.includes('Only a fully reported, Coordinator-completed separate re-test can close the original report.') && endpoint.includes("['closed', 'needs_reproduction', 'known_issue']"), "Closeout must follow reviewed re-test evidence and use only bounded original-report states.");

...
test("includes the authoritative What's new entries when supplied", () => {
  const notes = releaseNotesFromMarkdown([
    ...
    "## What's new",
    "",
    "- Adds Android Auto browsing and playback.",
    "- Preserves station artwork.",
    "",
    "## Validation",
    "",
  ...
  assert.match(messageFor(publishedAlpha, notes), /\*\*What's new\*\*[\s\S]*Android Auto browsing/);
  assert.doesNotMatch(messageFor(publishedAlpha, notes), /Not part of the announcement/);
});
